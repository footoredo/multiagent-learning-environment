...
class PackedActor(object):
    ...
    def add_key_pair(self, key, actor):
        self.keys.append(key)
        self.actors.append(actor)
        self.n += 1

    def strategy(self, belief, ob):
        s = np.zeros(self.n_slots)
        sw = 0.
        for i in range(self.n):
            w = weight(belief, self.keys[i])
            sw += w

        for i in range(self.n):
            w = weight(belief, self.keys[i])
            if w / sw > 0.01:
                ss = self.actors[i].strategy(ob)
                s += ss * (w / sw)
                # print(w / sw, ss)

        # print(belief, sw, np.sum(s))
        return s / np.sum(s)
...
class PPOAgentStacked(BaseAgent):
    ...
    def _load_sub(self, i, load_path):
        import json
        pi = PackedActor(self.n_slots)
        config = json.load(open(join_path(load_path, "pack-{}.json".format(i)), "r"))
        for j, entry in enumerate(config):
            key = entry["belief"]
            path = entry["path"]
            with tf.variable_scope(self.name):
                network = self.new_policy_network("subpi-{}-{}".format(i, j))
            load_variables(path, network.get_trainable_variables())
            pi.add_key_pair(np.array(key), network)
        return pi
    ...
    def _init(self, policy_fn, ob_space, ac_space,
        ...
        def new_policy_network(name):
            return policy_fn(name, self.name, ob_space, ac_space)
        self.new_policy_network = new_policy_network
        pi = new_policy_network("pi")  # Construct network for new policy
        oldpi = new_policy_network("oldpi")  # Network for old policy
        avgpi = new_policy_network("avgpi") # Network for avg policy
        self.subpis = [None for i in range(n_rounds - 1)]
    ...
    def divide_ob(self, ob):
        return ob[:self.n_types], self.get_round(ob[self.n_types:self.n_types + self.n_rounds]), ob[self.n_types + self.n_rounds:]
    ...
    def get_sub_strategy(self, i, belief, ob):
        return self.subpis[i].strategy(belief, ob)

    def _act(self, strategy):
        return np.random.choice(range(self.n_slots), p=strategy)
    ...
    def _traj_segment_generator(self, pi, env, horizon, stochastic):
        ...
        while True:
            ...
            belief, _, ob = self.divide_ob(ob)

            for j in range(self.n_rounds - 1):
                s = self.get_sub_strategy(j, belief, ob)
                ac = self._act(s)
                ob, sub_rew, _, new, prob, history = env.step(ac, my_prob)
                rew += sub_rew
                belief, _, ob = self.divide_ob(ob)
    ...
    def _get_policy(self, pi=None):
        ...
        def act_fn(ob):
            ...
            if r == 0:
                ...
                s = self.get_sub_strategy(r - 1, belief, ob)
                return self._act(s)
        ...
        def strategy_fn(ob):
            ...
            if r == 0:
                ...
                return self.get_sub_strategy(r - 1, belief, ob)

        return Policy(act_fn=act_fn, prob_fn=prob_fn, strategy_fn=strategy_fn)
