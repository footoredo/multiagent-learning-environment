...
class StackedMLPPolicy(object):
    recurrent = False

    def __init__(self, name, number_of_rounds, *args, **kwargs):
        self.name = "{}-{}".format(name, number_of_rounds)
        self.number_of_rounds = number_of_rounds
        if number_of_rounds > 1:
            self.next_solver = StackedMLPPolicy(name, number_of_rounds - 1, *args, **kwargs)
