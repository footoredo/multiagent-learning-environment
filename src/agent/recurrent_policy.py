...
class RecurrentPolicy(object):
    ...
    def _init(self, agent_name, init_ob_space, info_ob_space, ac_space, hid_size, num_hid_layers, gaussian_fixed_var=True):
        ...
        self._act = U.function([stochastic, init_ob, info_ob], [ac, self.vpred])
        # acq = tf.placeholder(dtype=tf.int32, shape=())
        # prob = tf.nn.softmax(self.pd.logits)[acq]
        # print(self.pd.logits)
        # print(self.pd.mean)
        prob = self.pd.mean
        self._prob = U.function([init_ob, info_ob], prob)
    ...
    def act(self, stochastic, ob):
        ...
        ac1, vpred1 = self._act(stochastic, init_ob[None], [info_ob])
        # print(ac1, vpred1)
    ...
    def prob(self, ob, ac):
        # print(ob)
        init_ob, info_ob = ob
        prob1 = self._prob(init_ob[None], info_ob[None])
        return prob1[0][ac]

    def strategy(self, ob):
        init_ob, info_ob = ob
        # print(ob, ob[None], self._prob(ob[None])[0])
        return self._prob(init_ob[None], info_ob[None])[0]
    ...
    def act_with_explore(self, stochastic, ob, explore_prob):
        ...
        if np.random.rand() < explore_prob:
            ...
            ac1, vpred1 = self._act(stochastic, init_ob[None], info_ob[None])
            return ac1[0], vpred1[0]
