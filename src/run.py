...
def make_ppo_agent(observation_space, action_space, handlers):
    ...
    agent = PPOAgent(name="ppo_agent_%d" % ppo_agent_cnt, policay_fn=policy,
                     ob_space=observation_space, ac_space=action_space, handlers=handlers,
                     timesteps_per_actorbatch=128, clip_param=0.2, entcoeff=0.0,
                     gamma=0.99, lam=0.95, max_episodes=1024)
    ppo_agent_cnt += 1
    return agent
...
if __name__ == "__main__":
    ...
    env = MatrixEnv()
    controller = NaiveController(env, [make_self_play_agent, make_self_play_agent])
    controller.train(max_steps=50000, update_handler=None)
