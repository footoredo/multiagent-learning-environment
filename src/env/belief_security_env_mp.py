...
class BeliefSecurityEnv(BaseEnv):
    ...
    def _convert_attacker_strategy(self, attacker_strategy):
        v = attacker_strategy.reshape(self.n_slots, -1, self.n_types)

        all_history = []

        def rec(h):
            if len(h) < self.n_rounds:
                all_history.append(h)
                for a in range(self.n_slots):
                    rec(h + [a])

        rec([])
        all_history = sorted(all_history, key=lambda x:len(x))
        assert len(all_history) == v.shape[1]

        s = [dict() for _ in range(self.n_types)]
        for i, h in enumerate(all_history):
            for t in range(self.n_types):
                s[t][self.encode_history(h)] = v[:, i, t]
        return s

    def _convert_defender_strategy(self, defender_strategy):
        v = defender_strategy.reshape(-1, self.n_slots)
        ...
        rec([])
        all_history = sorted(all_history, key=lambda x: len(x))
        assert len(all_history) == v.shape[0]

        s = dict()
        for i, h in enumerate(all_history):
            s[self.encode_history(h)] = v[i]
        return s
    ...
    def _assess_strategies(self, strategies):
        ...
        tas = self._convert_attacker_strategy(attacker_strategy)
        tds = self._convert_defender_strategy(defender_strategy)
