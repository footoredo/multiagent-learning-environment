...
class Scenario(BaseScenario):
    ...
    def agent_reward(self, agent, world):
        ...
        distance = np.sum(np.square(agent.state.p_pos - agent.goal_a.state.p_pos))
        shaping = -distance
        rew = 0.
    ...
    def adversary_reward(self, agent, world):
        ...
        #     if np.sqrt(np.sum(np.square(agent.state.p_pos - landmark.state.p_pos))) <= landmark.size + agent.size:
        #         # if np.sqrt(np.sum(np.square(good_agent.state.p_pos - landmark.state.p_pos))) <= landmark.size + good_agent.size:
        #         #     rew += 50.
        #         # else:
        #         #     rew += 100.
        #         break
        #
        # if np.sqrt(np.sum(np.square(agent.state.p_pos - agent.goal_a.state.p_pos))) <= agent.goal_a.size + agent.size:
        #     # if np.sqrt(np.sum(np.square(good_agent.state.p_pos - agent.goal_a.state.p_pos))) <= agent.goal_a.size + good_agent.size:
        #     #     rew += 200.
        #     # else:
        #     #     rew += 400.
        #     rew += 500.
