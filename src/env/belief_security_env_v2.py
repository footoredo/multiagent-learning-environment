...
class BeliefSecurityEnv(BaseEnv):
    def __init__(self, n_slots, n_types, prior, n_rounds, value_low=5., value_high=10., zero_sum=False, seed=None, record_def=False, export_gambit=False, random_prior=False):
        ...
        atk_ob_space = spaces.Box(low=0., high=1., shape=[n_types])
        ...
        # print(dfd_ob_space)
        atk_ac_space = spaces.MultiDiscrete([n_slots] * n_types)
        dfd_ac_space = spaces.Discrete(n_slots)
        super().__init__(num_agents=2,
                         observation_spaces=[atk_ob_space, dfd_ob_space],
                         action_spaces=[atk_ac_space, dfd_ac_space])
    ...
    def step(self, actions, action_probs):
        ...
        atk_action = np.random.choice(actions[0], p=self.belief)

        atk_rew = np.sum(self.payoff[:, atk_action, actions[1], 0] * self.belief)
        dfd_rew = self.payoff[0, atk_action, actions[1], 1]

        prob = np.array([self.policies[0].prob(self._get_atk_ob(t), atk_action) for t in range(self.n_types)])
        ...
        if self.rounds_so_far < self.n_rounds - 1:
            if self.record_def:
                ...
                self.ac_history[self.rounds_so_far][atk_action] = 1.
