...
class SecurityEnv(BaseEnv):
    def __init__(self, n_slots, n_types, prior, n_rounds, value_low=5., value_high=10., zero_sum=False, seed=None, record_def=False, export_gambit=False):
        ...
        atk_init_ob_space = spaces.Box(low=0., high=1., shape=[n_types])
        dfd_init_ob_space = spaces.Box(low=0., high=1., shape=[n_types])

        atk_info_ob_space = spaces.Box(low=0., high=1., shape=[n_slots])
        dfd_info_ob_space = spaces.Box(low=0., high=1., shape=[n_slots])

        atk_add_ob_space = spaces.Box(low=0., high=1., shape=[n_types + n_rounds])
        dfd_add_ob_space = spaces.Box(low=0., high=1., shape=[n_rounds])
    ...
    def _get_info_ob(self):
        return np.concatenate([np.zeros((1, self.n_slots)), self.ac_history[:self.rounds_so_far]], axis=0)

    def _get_atk_add_ob(self):
        return np.concatenate([one_hot(self.n_types, self.atk_type), one_hot(self.n_rounds, self.rounds_so_far)])

    def _get_dfd_add_ob(self):
        return one_hot(self.n_rounds, self.rounds_so_far)

    def _get_ob(self):
        return [(self._get_init_ob(), self._get_info_ob(), self._get_atk_add_ob()), (self._get_init_ob(), self._get_info_ob(), self._get_dfd_add_ob())]
    ...
    def get_type_space(self, i):
        return spaces.Discrete(self.n_types)

    def reset(self, debug=False):
        self.rounds_so_far = 0
        self.ac_history = np.zeros(shape=[self.n_rounds - 1, self.n_slots], dtype=np.float32)
        # if self.record_def:
    ...
    def get_ob_encoders(self):
        def atk_ob_encoder(ob):
            init_ob, info_ob, add_ob = ob
            return hash(tuple(add_ob[:self.n_types].astype(int).tolist() + np.array(info_ob).flatten().astype(int).tolist()))
    ...
    def convert_to_init_ob(self, prior=None):
        if prior is None:
            prior = self.prior
        return prior

    def convert_to_atk_add_ob(self, t, n):
        return np.concatenate([one_hot(self.n_types, t), one_hot(self.n_rounds, n)])

    def convert_to_dfd_add_ob(self, n):
        return one_hot(self.n_rounds, n)
    ...
    class _AttackerStrategyExploiter(object):
        def __init__(self, env):
            ...
            self._convert_to_init_ob = env.convert_to_init_ob
            self._convert_to_info_ob = env.convert_to_info_ob
            self._convert_to_atk_add_ob = env.convert_to_atk_add_ob
        ...
        def _recursive(self, history, prior):
            ...
            if encoded in self.cache:
                ...
                init_ob = self._convert_to_init_ob(self.prior)
                for t in range(self.n_types):
                    add_ob = self._convert_to_atk_add_ob(t, len(history))
                    atk_strategy = self.strategy((init_ob, info_ob, add_ob))
                    for i in range(self.n_slots):
    ...
    class _DefenderStrategyExploiter(object):
        def __init__(self, env):
            ...
            self._convert_to_init_ob = env.convert_to_init_ob
            self._convert_to_info_ob = env.convert_to_info_ob
            self._convert_to_dfd_add_ob = env.convert_to_dfd_add_ob
            self._encode_history = env.encode_history
            self.init_ob = self._convert_to_init_ob(self.prior)
        ...
        def _recursive(self, history, t):
            ...
            if encoded in self.cache:
                ...
                info_ob = self._convert_to_info_ob(history)
                add_ob = self._convert_to_dfd_add_ob(len(history))
                def_strategy = self.strategy((self.init_ob, info_ob, add_ob))
    ...
    class _DefenderUtilityCalculator(object):
        ...
        def _recursive(self, history, prior):
            ...
            if encoded in self.cache:
                ...
                for t in range(self.n_types):
                    atk_init_ob = self._convert_to_init_ob(self.prior)
                    atk_ob = self._convert_to_info_ob(history)
                    add_ob = self._convert_to_atk_add_ob(t, len(history))
                    atk_strategy = self.attacker_strategy((atk_init_ob, atk_ob, add_ob))
                    for i in range(self.n_slots):
                        atk_strategy_type[i][t] += atk_strategy[i] * prior[t]

                utility = 0.0
                def_init_ob = self._convert_to_init_ob(self.prior)
                def_ob = self._convert_to_info_ob(history)
                add_ob = self._convert_to_dfd_add_ob(len(history))
                def_strategy = self.defender_strategy((def_init_ob, def_ob, add_ob))
                for def_ac in range(self.n_slots):
    ...
    class _AttackerUtilityCalculator(object):
        ...
        def _recursive(self, history, t):
            ...
            if encoded in self.cache:
                ...
                atk_init_ob = self._convert_to_init_ob(self.prior)
                atk_ob = self._convert_to_info_ob(history)
                atk_add_ob = self._convert_to_atk_add_ob(t, len(history))
                atk_strategy = self.attacker_strategy((atk_init_ob, atk_ob, atk_add_ob))
                def_init_ob = self._convert_to_init_ob(self.prior)
                def_ob = self._convert_to_info_ob(history)
                def_add_ob = self._convert_to_dfd_add_ob(len(history))
                def_strategy = self.defender_strategy((def_init_ob, def_ob, def_add_ob))
                for def_ac in range(self.n_slots):
                    p_def = def_strategy[def_ac]
                    for atk_ac in range(self.n_slots):
