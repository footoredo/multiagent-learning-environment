...
def get_make_ppo_agent(timesteps_per_actorbatch, max_episodes):
    def make_ppo_agent(observation_space, action_space, handlers):
        ...
        agent = PPOAgent(name="ppo_agent_%d" % ppo_agent_cnt, policy_fn=policy,
                         ...
                         optim_epochs=1, optim_stepsize=5e-2,
...
if __name__ == "__main__":
    ...
    for _ in range(1):
        ...
        if train:
            n_ep = 2000
            t_ev = 10
            controller = NaiveController(env, [get_make_ppo_agent(1, 20), get_make_ppo_agent(1, 20)])
            results = controller.train(max_steps=n_ep, policy_store_every=None, test_every=t_ev, test_max_steps=100)
            eob = env.get_ob_encoders()[0](np.zeros(1))
            for i in range(t_ev, n_ep, t_ev):
                a0p0 = results[i // t_ev - 1][0][eob][0]
                a0p1 = results[i // t_ev - 1][0][eob][1]
                a1p0 = results[i // t_ev - 1][1][eob][0]
                a1p1 = results[i // t_ev - 1][1][eob][1]
                ...
                res["player"].append("Player 1 regret")

                res["episode"].append(i)
                res["regret"].append(regret_1)
                res["prob"].append(a1p0)
                res["player"].append("Player 2 regret")
    ...
    # pickle.dump(res, open("wolf.obj", "wb"))
